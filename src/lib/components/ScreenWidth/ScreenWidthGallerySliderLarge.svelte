<script lang="ts">
  ...
  $effect(() => {
    ...
    if (sliderIndex == -1) progressWrapBackwardPosition = itemArray.length * 100 - 100;
...
<section class="pb-32 {className || ''}">
  <div use:swipe class="h-py-2 relative" style="height:{imageWidth * 0.95}px;">
    <div
      class="h-full flex flex-row flex-nowrap {isSlideAnimated ? 'transition-transform duration-[2000ms]' : ''}"
      style="width:{(imageWidth - 8) * tripledItems.length}px; margin-left:calc(50vw - {(imageWidth - 8) /
